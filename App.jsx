...
import { Book, MessageSquare, Send, Crown, Bell, Sparkles, LogOut, Trash2, ChevronLeft, ChevronRight, X, Info, Heart, Camera, Settings } from 'lucide-react';
...
export default function App() {
  ...
  const handlePhotoUpload = (e) => {
    ...
    if (file) {
      if (file.size > 800000) return notify("Imagem muito pesada! (Máx 800kb)");
  ...
  const handleAddManga = async (e) => {
    ...
    const pInput = f.get('pages');
    const pages = pInput ? pInput.split(',').map(p => p.trim()).filter(p => p !== "") : [];
  ...
  const toggleLikeManga = async (mId) => {
    ...
    if (!manga) return;
    const likes = manga.likes || [];
    const hasLiked = likes.includes(user.uid);
    await updateDoc(mRef, { likes: hasLiked ? arrayRemove(user.uid) : arrayUnion(user.uid) });
  };

  if (isLoading) return <div className="h-screen bg-black flex flex-col items-center justify-center gap-4">
    <Sparkles className="text-blue-600 w-12 h-12 animate-spin" />
    <span className="text-blue-500 font-black uppercase tracking-[0.3em]">Sincronizando Sistema...</span>
  </div>;

  return (
    <div className="min-h-screen bg-[#050505] text-slate-200 selection:bg-blue-600/30">
      
      {/* NOTIFICAÇÃO */}
      {notification && (
        <div className="fixed top-24 right-6 z-[100] bg-blue-600 text-white px-6 py-3 rounded-2xl shadow-2xl animate-in slide-in-from-top-10 border border-blue-400">
      ...
      {showProfileModal && (
        <div className="fixed inset-0 z-[110] bg-black/95 backdrop-blur-md flex items-center justify-center p-6">
          <div className="bg-[#0a0a0a] border border-white/5 w-full max-w-md rounded-[40px] p-10 space-y-8 relative">
            <X className="absolute top-8 right-8 cursor-pointer text-slate-500" onClick={() => setShowProfileModal(false)} />
            <h2 className="text-2xl font-black text-white uppercase flex items-center gap-3"><Settings className="text-blue-500" /> Identidade</h2>
            <div className="flex flex-col items-center gap-4">
              ...
              />
              <button onClick={handleUpdateProfile} className="w-full bg-blue-600 py-4 rounded-xl font-black uppercase tracking-widest hover:bg-blue-500 transition-all">Salvar</button>
            </div>
          </div>
        </div>
      )}
          ...
          {!user ? (
            ...
            <div className="flex items-center gap-3">
              <img 
                ...
                className="w-9 h-9 rounded-xl border border-blue-500/30 object-cover cursor-pointer hover:scale-105 transition-all" 
              />
      ...
      <main className="max-w-7xl mx-auto p-6">
        {view === 'home' && (
          <div className="space-y-12">
            <div className="rounded-[40px] bg-blue-900/5 border border-white/5 p-16 text-center mt-4">
              ...
              <p className="text-slate-500 font-bold uppercase tracking-[0.5em] text-[9px] opacity-60">DOMÍNIO SOBERANO V4.2</p>
            </div>

            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-8">
              {mangas.map(m => (
                <div key={m.id} className="group relative" onClick={() => { setSelectedManga(m); setView('detail'); setCurrentPage(0); }}>
                  <div className="aspect-[3/4.5] rounded-3xl overflow-hidden border border-white/10 group-hover:border-blue-500 transition-all shadow-2xl cursor-pointer bg-slate-900">
        ...
        {view === 'detail' && selectedManga && (
          <div className="space-y-12 mt-6">
            <button onClick={() => setView('home')} className="flex items-center gap-2 text-[10px] font-black uppercase text-slate-500 hover:text-white"><ChevronLeft className="w-4 h-4" /> Voltar</button>
                ...
                {selectedManga.pages?.length > 0 ? (
                  <div className="space-y-6 pt-10 border-t border-white/5">
                    ...
                    <div className="flex items-center justify-center gap-8">
                      <ChevronLeft className="cursor-pointer hover:text-blue-500" onClick={() => setCurrentPage(p => Math.max(0, p - 1))} />
                      ...
                      <ChevronRight className="cursor-pointer hover:text-blue-500" onClick={() => setCurrentPage(p => Math.min(selectedManga.pages.length - 1, p + 1))} />
                    </div>
                  </div>
                ) : <div className="p-10 border border-white/5 rounded-3xl text-center text-[10px] uppercase font-black opacity-30 tracking-widest">Nenhuma página disponível.</div>}
                ...
                <section className="pt-10 border-t border-white/5 space-y-6">
                  ...
                  <div className="flex gap-4">
                    ...
                    <button onClick={() => {
                      ...
                      updateDoc(mRef, { comments: arrayUnion(newComment) });
                      setCommentText('');
                      notify("Enviado!");
                    }} className="bg-blue-600 p-4 rounded-2xl disabled:opacity-20" disabled={!user || !commentText.trim()}><Send className="w-5 h-5" /></button>
                  </div>
                    ...
                    {selectedManga.comments?.map((c) => (
                      <div key={c.id} className="bg-white/5 p-5 rounded-3xl border border-white/5 relative group">
                        ...
                        {(isMonarch || user?.uid === c.userId) && (
                          <Trash2 className="absolute top-5 right-5 w-4 h-4 text-slate-500 hover:text-red-500 cursor-pointer opacity-0 group-hover:opacity-100 transition-opacity" onClick={async () => {
                            const mRef = doc(db, 'artifacts', appId, 'public', 'data', 'mangas', selectedManga.id);
      ...
      <footer className="py-20 text-center opacity-30 text-[9px] font-black uppercase tracking-[0.5em] border-t border-white/5 mt-20">Lumi Mangás • Monarca Jean • 2026</footer>
    </div>
  );
}
